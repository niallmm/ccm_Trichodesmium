...
figure(222)
loglog(pHoutv, frac_oxygenation_ccm, 'b');
hold on;
loglog(pHoutv, frac_oxygenation_cy, 'g');
plot(pHoutv, frac_oxygenation_just_rbc, 'r');
...
if plot1 == 1
    ...
    figure(5)
    subplot(1,3,1);
    area(pHoutv, total_cost_ccm_h, 'FaceColor', [.337,.165,.447], 'BaseValue', 1e-1);
    hold on;
    area(pHoutv, cost_fixation + cost_2pg_ccm_h, 'FaceColor', [.251, .557, .184], 'BaseValue', 1e-1);
    area(pHoutv, cost_2pg_ccm_h, 'FaceColor', [.667, .518, .224], 'BaseValue', 1e-1);
    plot(pHoutv, total_cost_ccm_h, '-k', 'LineWidth', 3);
    set(gca, 'Yscale', 'log');
    axis([pHoutv(1) pHoutv(end) 1e-1 2e8]);
    ylabel('Energetic Cost H^+ / (CO_2 fixed)');
    xlabel('external pH');
    title('Full CCM');
    
    % Cost breakdown of the cytosolic enzymes model as a function of pHoutv.
    subplot(1,3,2);
    area(pHoutv, total_cost_cy_h, 'FaceColor', [.337,.165,.447], 'BaseValue', 1e-1);
    hold on;
    area(pHoutv, cost_fixation + cost_2pg_cy_h, 'FaceColor', [.667, .518, .224], 'BaseValue', 1e-1);
    area(pHoutv, all_ones .* cost_fixation, 'FaceColor', [.251, .557, .184], 'BaseValue', 1e-1);
    plot(pHoutv, total_cost_cy_h, '-k', 'LineWidth', 3);
    set(gca, 'Yscale', 'log');
    axis([pHoutv(1) pHoutv(end) 1e-1 2e8]);
    set(gca, 'YTick', []);
    xlabel('external pH');
    ...
    subplot(1,3,3);
    area(pHoutv, total_cost_just_rbc_h, 'FaceColor', [.337,.165,.447], 'BaseValue', 1e-1);
    hold on;
    area(pHoutv, cost_fixation + cost_2pg_just_rbc_h, 'FaceColor', [.667, .518, .224], 'BaseValue', 1e-1);
    area(pHoutv, all_ones .* cost_fixation, 'FaceColor', [.251, .557, .184], 'BaseValue', 1e-1);
    plot(pHoutv, total_cost_just_rbc_h, '-k', 'LineWidth', 3);
    set(gca, 'Yscale', 'log');
    axis([pHoutv(1) pHoutv(end) 1e-1 2e8]);
    set(gca, 'YTick', []);
    xlabel('external pH');
...
loglog(pHoutv, cost_h,'Color','r', 'LineWidth', 3);
...
semilogy(pHoutv, cost_2pg_ccm_h, 'Color','k', 'LineWidth', 3);
text(6, 1, 'Cost of photorespiration', 'Color', 'k')
%plot cost of pH maintenance
semilogy(pHoutv, cost_pH_maintenance, '--c', 'LineWidth',3)
title('Full CCM')
% 
...
if plot3 ==1
    figure(111)
    plot(pHoutv, Ccsome, 'r')
    hold on
    plot(pHoutv, Hcsome, 'b')
    xlabel('external pH');
