...
figure(222)
loglog(Houtv, frac_oxygenation_ccm, 'b');
hold on;
loglog(Houtv, frac_oxygenation_cy, 'g');
plot(Houtv, frac_oxygenation_just_rbc, 'r');
...
if plot1 == 1
    ...
    figure(5)
    subplot(1,3,1);
    area(Houtv, total_cost_ccm_h, 'FaceColor', [.337,.165,.447], 'BaseValue', 1e-1);
    hold on;
    area(Houtv, cost_fixation + cost_2pg_ccm_h, 'FaceColor', [.251, .557, .184], 'BaseValue', 1e-1);
    area(Houtv, cost_2pg_ccm_h, 'FaceColor', [.667, .518, .224], 'BaseValue', 1e-1);
    plot(Houtv, total_cost_ccm_h, '-k', 'LineWidth', 3);
    set(gca, 'Yscale', 'log');
    axis([Houtv(1) Houtv(end) 1e-1 2e8]);
    ylabel('Energetic Cost H^+ / (CO_2 fixed)');
    xlabel('External HCO_3^-');
    title('Full CCM');
    
    % Cost breakdown of the cytosolic enzymes model as a function of Houtv.
    subplot(1,3,2);
    area(Houtv, total_cost_cy_h, 'FaceColor', [.337,.165,.447], 'BaseValue', 1e-1);
    hold on;
    area(Houtv, cost_fixation + cost_2pg_cy_h, 'FaceColor', [.667, .518, .224], 'BaseValue', 1e-1);
    area(Houtv, all_ones .* cost_fixation, 'FaceColor', [.251, .557, .184], 'BaseValue', 1e-1);
    plot(Houtv, total_cost_cy_h, '-k', 'LineWidth', 3);
    set(gca, 'Yscale', 'log');
    axis([Houtv(1) Houtv(end) 1e-1 2e8]);
    set(gca, 'YTick', []);
    xlabel('External HCO_3^-');
    ...
    subplot(1,3,3);
    area(Houtv, total_cost_just_rbc_h, 'FaceColor', [.337,.165,.447], 'BaseValue', 1e-1);
    hold on;
    area(Houtv, cost_fixation + cost_2pg_just_rbc_h, 'FaceColor', [.667, .518, .224], 'BaseValue', 1e-1);
    area(Houtv, all_ones .* cost_fixation, 'FaceColor', [.251, .557, .184], 'BaseValue', 1e-1);
    plot(Houtv, total_cost_just_rbc_h, '-k', 'LineWidth', 3);
    set(gca, 'Yscale', 'log');
    axis([Houtv(1) Houtv(end) 1e-1 2e8]);
    set(gca, 'YTick', []);
    xlabel('External HCO_3^-');
...
loglog(Houtv, cost_h,'Color','r', 'LineWidth', 3);
axis([Houtv(1) Houtv(end) 1e-1 2e7])
hold on
ylabel('Energetic Cost H^+ / (CO_2 fixed)')
    xlabel('External HCO_3^-');
text(6, 5e2, 'HCO3^- transport cost', 'Color', 'r')
% plot cost of fixation/CO2 (flat)
plot([Houtv(1) Houtv(end)], [cost_fixation cost_fixation], '--b');
text(3, 20, 'Calvin Cycle Fixation', 'Color', 'b');
% plot cost of photorespiration
semilogy(Houtv, cost_2pg_ccm_h, 'Color','k', 'LineWidth', 3);
text(6, 1, 'Cost of photorespiration', 'Color', 'k')
%plot cost of pH maintenance
semilogy(Houtv, cost_pH_maintenance, '--c', 'LineWidth',3)
title('Full CCM')
% 
...
subplot(1,3,2);
% plot cost of transport/CO2 fixation with pH
loglog(Houtv, cost_cy_h,'Color','r', 'LineWidth', 3);
axis([Houtv(1) Houtv(end) 1e-1 2e7])
hold on
ylabel('Energetic Cost H^+ / (CO_2 fixed)')
    xlabel('External HCO_3^-');
...
text(3, 20, 'Calvin Cycle Fixation', 'Color', 'b');
% plot cost of photorespiration
loglog(Houtv, cost_2pg_cy_h, 'Color','k', 'LineWidth', 3);
text(6, 5e2, 'Cost of photorespiration', 'Color', 'k')
title('Reactions in Cytosol')
subplot(1,3,3);
% plot cost of transport/CO2 fixation with pH
loglog(Houtv, cost_just_rbc_h,'Color','r', 'LineWidth', 3);
axis([Houtv(1) Houtv(end) 1e-1 2e7])
hold on
ylabel('Energetic Cost H^+ / (CO_2 fixed)')
xlabel('External HCO_3^-');
...
text(3, 20, 'Calvin Cycle Fixation', 'Color', 'b');
% plot cost of photorespiration
loglog(Houtv, cost_2pg_just_rbc_h, 'Color','k', 'LineWidth', 3);
